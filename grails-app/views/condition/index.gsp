		...
		<div id="list-condition" class="content scaffold-list" role="main">
			...
			<thead>
					<tr>
						...
						<g:sortableColumn property="title" title="${message(code: 'condition.title.label', default: 'Title')}" />
					
						<g:sortableColumn property="filePayload" title="${message(code: 'condition.filePayload.label', default: 'File Payload')}" />
				...
				<g:each in="${conditionInstanceList}" status="i" var="conditionInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${conditionInstance.id}">${fieldValue(bean: conditionInstance, field: "title")}</g:link></td>
					
						<td>${fieldValue(bean: conditionInstance, field: "filePayload")}</td>
